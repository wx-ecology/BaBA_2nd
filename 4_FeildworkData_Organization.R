...
GPS <- st_read("./data/fieldwork/Field_Data/raw_results/eTrex_all_Pts.kml") %>% 
  select (-Description)
GPS <- GPS %>%
...
meas.pts <- read_csv("./data/fieldwork/Field_Data/MeaurementPts.csv") %>% 
  select (-'Point ID', -measurements) %>% filter (!is.na(Structure)) %>% 
  left_join(GPS, by = "GPS_ID") %>% relocate(GPS_ID, lat, lon)
write_csv(meas.pts, "./data/fieldwork/Field_Data/FieldMeasPts_clean.csv")

valid.pts <- read_csv("./data/fieldwork/Field_Data/ValidationPts.csv") %>%
